...
def extract_binary_segments(
    ...
    Returns (windows, labels, start_samples): windows shape (n_windows,
    n_channels, window_samples), labels shape (n_windows,), 1=ictal/
    0=interictal, start_samples shape (n_windows,) -- the sample index
    each window begins at in the ORIGINAL continuous recording, needed
    by build_subject_dataset_chbmit to pool the right region from a
    CQT computed once on the full continuous signal (see that
    function's docstring for why the CQT itself cannot be computed
    per-window). Raises if a declared seizure window is out of the
    recording's bounds -- this would indicate a parsing or alignment
    ...
    return np.stack(windows), np.array(labels, dtype=int), np.array(start_samples, dtype=int)
...
def build_subject_dataset_chbmit(
    ...
    windows, labels, start_samples = extract_binary_segments(
    ...
    # CQT computed ONCE on the full continuous per-channel signal (see
    # model/cqt_encoder.py's own documented constraint: fmin=1Hz needs
    # ~16.8s of context, far more than a 4s window) -- confirmed the
    # hard way this session: computing it per-window crashed with a
    # padding error on a 1024-sample (4s @ 256Hz) input. Only the
    # connectivity/Laplacian step below still operates per-window.
    ...
    for window_signal, start_sample in zip(windows, start_samples):
        per_channel_pooled = [
            pool_cqt_to_node_features(
                cqt_per_channel[ci], int(start_sample), sfreq, hop_length, tmin=0.0, tmax=window_s,
            )
            for ci in range(n_channels)
        ]
